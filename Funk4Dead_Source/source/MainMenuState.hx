...
class MainMenuState extends MusicBeatState
{
	...
	override function update(elapsed:Float)
	{
        ...
        // Mover la cámara hacia el cursor
        // Asegúrate de que la cámara no se salga de los límites
        FlxG.camera.setPosition(
            Math.max(0, Math.min(mouseX - FlxG.camera.width / 2, FlxG.worldBounds.width - FlxG.camera.width)), 
            Math.max(0, Math.min(mouseY - FlxG.camera.height / 2, FlxG.worldBounds.height - FlxG.camera.height))
        );
		...
		menuItems.forEach(function(spr:FlxSprite)
		{
			...
			if (spr.ID == curSelected){
				var mult = FlxMath.lerp(spr.scale.x, 1.23, lerpvalue);
